...
This folder provides a lightweight reference deployment shape for the Chronogram application layer.
...
```bash
kubectl create secret generic chronogram-secrets \
  --namespace chronogram \
  --from-literal=neo4j-password='<replace-me>' \
  --from-literal=api-key='<replace-me>'
```

The reference app manifests wire `CHRONOGRAM_API_KEY` from this secret. Health endpoints stay open for Kubernetes probes; all other API and worker routes require auth.
...
- `CHRONOGRAM_JWT_ISSUER`
- `CHRONOGRAM_JWT_AUDIENCE`
- optional `CHRONOGRAM_JWKS_URL`
- optional `CHRONOGRAM_JWT_REQUIRED_SCOPES`
