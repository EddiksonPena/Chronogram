...
This guide explains how to set up `Chronogram`, launch the local stack, and exercise the system end to end.
...
curl -H 'content-type: application/json' http://127.0.0.1:8080/v1/graphql \
  -d '{"query":"{ Get { ChronogramEpisodicChunk(limit: 5) { artifactId scope content } } }"}'
```

### Neo4j

```bash
docker exec chronogram-knowledge-graph-store-1 cypher-shell -u neo4j -p chronogram \
