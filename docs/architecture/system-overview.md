...
`Chronogram` is a memory operating system for agent ecosystems.
...
2. When pressure crosses a threshold, Chronogram creates a compaction plan.
...
Chronogram now treats working memory as explicitly compactable rather than infinitely accumulating.
