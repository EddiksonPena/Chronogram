...
This runbook defines the minimum backup and restore expectations for a production-grade `Chronogram` deployment.
...
`Chronogram` application pods are stateless in the deployment reference and do not require direct filesystem backup for core memory state.
...
3. Verify that `chronogram:state:v1` exists.
