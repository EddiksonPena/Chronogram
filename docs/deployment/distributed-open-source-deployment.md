...
Turn `Chronogram` from a strong local-first prototype into a robust distributed open source project that can be operated by a small team and adopted by the community.
...
6. Turn on `CHRONOGRAM_API_KEY` before exposing routes beyond trusted local networks.
