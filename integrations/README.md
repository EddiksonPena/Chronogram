# Chronogram Integrations

Chronogram works with **every major AI agent harness**. One MCP server, six adapters — same persistent memory everywhere.
...
| Tool | Description |
|------|-------------|
| `chronogram_health` | Check if Chronogram is reachable |
| `chronogram_remember` | Store a fact, decision, or discovery |
| `chronogram_recall` | Semantic search across all memory |
| `chronogram_compact` | Compress conversation into durable memory |
...
cp integrations/adapters/codex/chronogram-memory.md ./
```
...
1. **Chronogram running:** `docker compose up -d` from monorepo root
