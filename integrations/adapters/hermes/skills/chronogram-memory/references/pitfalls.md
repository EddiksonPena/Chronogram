...
| Neo4j auth fails | NEO4J_PASSWORD in .env | Set to `chronogram-local` or match docker-compose |
...
Chronogram uses file-backed state by default (`./data/chronogram-memory-os.json`).

```bash
# Backup
cp ~/projects/chronogram/data/chronogram-memory-os.json ~/backups/chronogram-$(date +%Y%m%d).json

# Reset (careful: loses all memory)
rm ~/projects/chronogram/data/chronogram-memory-os.json
