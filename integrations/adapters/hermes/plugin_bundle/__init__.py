"""
Chronogram Memory OS — Hermes plugin.
...
Hooks registered:
  ...
  post_tool_call  — persist tool calls to Chronogram as episodic memories
...
MCP_CHRONO_TOOLS = {"mcp_chronogram_health", "mcp_chronogram_remember",
...
def _on_session_start(
    ...
    **kwargs,
    ...
    """Validate Chronogram connectivity + recall user/project context."""
    try:
        with httpx.Client(timeout=REQUEST_TIMEOUT) as client:
            health = _get_json(client, "/health")
            if health and health.get("status") == "ok":
                mem = health.get("memory", {})
                logger.info("Chronogram OK — %s artifacts", mem.get("artifactCount", "?"))
            else:
                logger.warning("Chronogram unavailable at session start")
    except Exception:
        pass
...
def _on_session_end(
    ...
    **kwargs,
    ...
    """Mark session end in Chronogram."""
...
def _pre_llm_call(
    ...
    **kwargs,
    ...
    """
    Inject recalled context from Chronogram + capture conversation state.
    ...
    if recalls:
        return "Recalled from Chronogram:\n" + "\n".join(recalls)
    return None
...
def _post_tool_call(
    ...
    **kwargs,
    ...
    """Capture every tool call to Chronogram as an episodic memory."""
...
def _post_llm_call(
    ...
    **kwargs,
    ...
    """After a completed turn, persist the dialog to Chronogram."""
...
def register(ctx) -> None:
    """Register all Chronogram hooks. Called once by the plugin loader."""
    ctx.register_hook("on_session_start", _on_session_start)
    ctx.register_hook("on_session_end", _on_session_end)
    ctx.register_hook("pre_llm_call", _pre_llm_call)
    ctx.register_hook("post_tool_call", _post_tool_call)
    ctx.register_hook("post_llm_call", _post_llm_call)
    logger.info("Chronogram plugin registered — 5 hooks active")
