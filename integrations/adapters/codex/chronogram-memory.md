...
- `chronogram_remember(scope, content, source?, tags?)` — Store a memory
- `chronogram_recall(query, scope?)` — Search memory semantically
- `chronogram_compact(session_id, messages, scope?)` — Compact conversation
- `chronogram_feedback(artifact_id, useful)` — Reinforce/demote
