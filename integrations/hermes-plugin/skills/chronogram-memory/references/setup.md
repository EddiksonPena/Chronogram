# Chronogram Setup Guide

Full installation and configuration for Chronogram memory OS.
...
curl -s -X POST http://127.0.0.1:4000/v1/memories/ingest \
  ...
  -d '{
    ...
    "source": "setup-test",
    "tags": ["test"],
    "content": "Chronogram is running on port 4000 and the worker is on 4010."
  }'
...
curl -s -X POST http://127.0.0.1:4000/v1/memories/recall \
  -H 'content-type: application/json' \
  -d '{
    "query": "What port is Chronogram running on?",
    "scope": "workspace"
  }' | python3 -m json.tool
```
