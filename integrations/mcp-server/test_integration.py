"""End-to-end test of Chronogram MCP Server against live Chronogram."""
...
async def test_all():
    ...
    async with Client("http://127.0.0.1:8000/mcp") as client:
        # 1 — Health
        try:
            r = await client.call_tool("chronogram_health", {})
        ...
        # 2 — Remember
        try:
            r = await client.call_tool("chronogram_remember", {
                "scope": "user-profile",
        ...
        # 3 — Recall
        try:
            r = await client.call_tool("chronogram_recall", {
        ...
        # 4 — Compact
        try:
            r = await client.call_tool("chronogram_compact", {
        ...
        # 5 — List
        try:
            r = await client.call_tool("chronogram_list", {"scope": "user-profile"})
            text = r.content[0].text
            data = json.loads(text)
