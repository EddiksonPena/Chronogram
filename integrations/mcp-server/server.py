"""
Chronogram MCP Server — exposes Chronogram memory API as native MCP tools.
Connects to Chronogram on localhost:4000.
...
@mcp.tool
async def chronogram_health() -> dict:
...
@mcp.tool
async def chronogram_feedback(artifact_id: str, useful: bool) -> dict:
...
@mcp.tool
async def chronogram_list(scope: str | None = None) -> dict:
