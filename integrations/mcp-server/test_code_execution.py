"""End-to-end test of code-execution pattern with generated Chronogram wrappers.
...
import os
server_dir = os.path.join(os.path.dirname(__file__), "mcp_servers", "chronogram")
...
from mcp_servers.chronogram import (
    ...
    chronogram_remember_sync,
    chronogram_recall_sync,
    chronogram_compact_sync,
    chronogram_feedback_sync,
    chronogram_list_sync,
)

# Step 3: Health check
health = chronogram_health_sync()
print(f"\n1. Health: {health['status']} — {health['memory']['artifactCount']} artifacts")

# Step 4: Store a new fact
r = chronogram_remember_sync(
    scope="workspace",
    content="MCP code-execution wrappers generated successfully for Chronogram. Context usage down 98.7%.",
...
results = chronogram_recall_sync("What pattern reduces MCP context usage?")
...
if ctx:
    f = chronogram_feedback_sync(artifact_id=ctx[0]["id"], useful=True)
    print(f"4. Feedback: updated={f.get('updated')}")

# Step 7: Compact a micro-session
c = chronogram_compact_sync(
...
memories = chronogram_list_sync(scope="workspace")
