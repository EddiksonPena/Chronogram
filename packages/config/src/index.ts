...
export interface AppConfig {
  ...
  chronogramAuthMode: "none" | "api-key" | "jwt" | "hybrid";
  chronogramJwtIssuer: string | undefined;
  chronogramJwtAudience: string | undefined;
  chronogramJwksUrl: string | undefined;
  chronogramJwtRequiredScopes: string[];
  ...
  chronogramApiKey: string | undefined;
}
...
export const loadConfig = (
  ...
  const memoryDataFile = resolvePath(
    baseDir,
    env.MEMORY_DATA_FILE ?? `${memoryDataDir}/chronogram-memory-os.json`,
  );
  const apiKey = env.CHRONOGRAM_API_KEY?.trim() || undefined;
  const jwtIssuer = env.CHRONOGRAM_JWT_ISSUER?.trim() || undefined;
  const jwtAudience = env.CHRONOGRAM_JWT_AUDIENCE?.trim() || undefined;
  const jwksUrl = env.CHRONOGRAM_JWKS_URL?.trim() || undefined;
  const authMode =
    env.CHRONOGRAM_AUTH_MODE === "none"
      ? "none"
      : env.CHRONOGRAM_AUTH_MODE === "jwt"
        ? "jwt"
        : env.CHRONOGRAM_AUTH_MODE === "hybrid"
          ? "hybrid"
          : env.CHRONOGRAM_AUTH_MODE === "api-key"
            ? "api-key"
            : jwtIssuer || jwksUrl
              ? apiKey
                ? "hybrid"
  ...
  return {
    ...
    memoryStateRedisKey: env.MEMORY_STATE_REDIS_KEY ?? "chronogram:state:v1",
    chronogramAuthMode: authMode,
    chronogramJwtIssuer: jwtIssuer,
    chronogramJwtAudience: jwtAudience,
    chronogramJwksUrl: jwksUrl,
    chronogramJwtRequiredScopes: parseCsv(env.CHRONOGRAM_JWT_REQUIRED_SCOPES),
    ...
    neo4jPassword: env.NEO4J_PASSWORD ?? "chronogram",
    temporalAddress: env.TEMPORAL_ADDRESS ?? "localhost:7233",
    temporalNamespace: env.TEMPORAL_NAMESPACE ?? "default",
    temporalWorkflowTaskQueue: env.TEMPORAL_WORKFLOW_TASK_QUEUE ?? "chronogram-memory-module",
