...
import type { AppConfig } from "@chronogram/config";
...
export const createRequestAuthorizer = (config: AppConfig) => {
  const jwksUrl = config.chronogramJwksUrl
    ?? (config.chronogramJwtIssuer ? new URL("/.well-known/jwks.json", config.chronogramJwtIssuer).toString() : undefined);
  ...
  const authorizeJwt = async (req: IncomingMessage): Promise<AuthorizationResult> => {
    ...
    try {
      const verification = await jwtVerify(token, jwks, {
        ...(config.chronogramJwtIssuer ? { issuer: config.chronogramJwtIssuer } : {}),
        ...(config.chronogramJwtAudience ? { audience: config.chronogramJwtAudience } : {}),
      });

      const scopes = parseScopes(verification.payload as Record<string, unknown>);
      const missingScopes = config.chronogramJwtRequiredScopes.filter((scope) => !scopes.includes(scope));
