...
const preflight = async () => {
  ...
  const authMode = values.get("CHRONOGRAM_AUTH_MODE")?.trim() || "none";
  ...
  addCheck(
    ...
    authMode !== "none",
    authMode !== "none"
      ? `CHRONOGRAM_AUTH_MODE is ${authMode}.`
      : "CHRONOGRAM_AUTH_MODE is none; production deployments should require auth.",
  );
  ...
  if (authMode === "api-key" || authMode === "hybrid") {
    requireValue(checks, warnings, values, "CHRONOGRAM_API_KEY");
  }

  if (authMode === "jwt" || authMode === "hybrid") {
    requireValue(checks, warnings, values, "CHRONOGRAM_JWT_ISSUER");
    requireValue(checks, warnings, values, "CHRONOGRAM_JWT_AUDIENCE");

    const jwksUrl = values.get("CHRONOGRAM_JWKS_URL")?.trim()
      || (() => {
        const issuer = values.get("CHRONOGRAM_JWT_ISSUER")?.trim();
...
const smoke = async () => {
  ...
  if (!apiKey && !bearerToken) {
    addCheck(checks, "auth-credentials", false, "Provide CHRONOGRAM_API_KEY or CHRONOGRAM_BEARER_TOKEN for authenticated smoke tests.");
...
const load = async () => {
  ...
  if (!apiKey && !bearerToken) {
    print({
      ok: false,
      message: "Provide CHRONOGRAM_API_KEY or CHRONOGRAM_BEARER_TOKEN for authenticated load checks.",
    });
    process.exit(1);
  }
...
if (command === "help" || command === "--help" || command === "-h") {
  print([
    "Chronogram production readiness commands",
    "",
